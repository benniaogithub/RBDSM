...
def sentCombMat_R1(w1,w2,w3):  #主谓宾 nsubj 25  dobj 18  分别把主语和宾语的相应项变成0
    ...
    w1MatN = np.zeros(w1Mat.shape,dtype=np.float64)
    w1MatN[24] = np.zeros(w1Mat.shape[1])
    w2Mat = sio.loadmat(root+u"l_"+w2)[w2]
    w2MatN = w2Mat

    w3Mat = sio.loadmat(root+u"l_"+w3)[w3]
    w3MatN = np.zeros(w1Mat.shape,dtype=np.float64)
    w3MatN[17] = np.zeros(w3Mat.shape[1])
    # w3Mat[17] = np.zeros(w1Mat.shape[1])

    # return w1Mat+w2Mat+w3Mat
    return w1MatN+w2MatN+w3MatN
...
def emnlp2013_R1():        #三个词组成的句子，向量相加 ,计算相似度按行
    ...
    for line in open(u"res/emnlp2013_turk_RBDSM.txt"):
        ...
        w22 = item[5]
        w23 = item[6]
        sim1 = wordlantCosSimCol(sentCombMat_R1(w11,w12,w13),sentCombMat_R1(w21,w22,w23))
        sim2 = item[7]
        # print sim1
        wslist.append(sim1)
