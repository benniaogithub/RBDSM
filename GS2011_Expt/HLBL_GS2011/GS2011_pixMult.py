...
def creSampStep_1():              #提取前三列 其他不要了
    ...
    rfile.close()
    wfile = open(u"res/GS2011_HLBL.txt","w")
...
def GS2011_Mult():        #三个词组成的句子，向量相加 ,计算相似度按行

    wslist = []
    sdsmlist = []
    count = 1

    for line in open(u"res/GS2011_HLBL.txt"):
        item = line.strip("\n").split(" ")
        w11 = item[1]
        w12 = item[2]
        w13 = item[3]
        w21 = item[4]
        sim1 = vecSim(sentCombMat_Mult(w11,w12,w13),sentCombMat_Mult(w21,w12,w13))
        sim2 = item[5]
        wslist.append(sim1)
