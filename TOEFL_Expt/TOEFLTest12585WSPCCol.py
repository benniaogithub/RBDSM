...
@time: 2016/4/28 3:53
"""
import wordPairSim12585wSPCCol as wordPairSim                #引入不同的wordpairSim
import numpy as np
def getWordList():
...
for line in open("../res/TOEFL.txt"):
    ...
    if choice4 not in wordList:
        ...
        continue
    total += 1
    chosen = np.argmax([wordPairSim.wordlantCosSim(word,choice1),wordPairSim.wordlantCosSim(word,choice2),wordPairSim.wordlantCosSim(word,choice3),wordPairSim.wordlantCosSim(word,choice4)])
    print(chosen)
    if(chosen == 0):
        count += 1
print(count)
